...
function growthTimelinePanel() {
  ...
  return `<section class="growth-timeline simple-timeline unified-growth" aria-labelledby="growth-timeline-title">
    <header><h2 id="growth-timeline-title">Growth Plan</h2></header>
    <ol class="strategy-flow" aria-label="Growth strategy flow">
      <li><span class="strategy-flow-label">What we know</span><h3>Real use matters</h3><p>Learn who returns and pays.</p></li>
      <li><span class="strategy-flow-label">Who we’re looking for</span><h3>Android creators</h3><p>People making reaction videos.</p></li>
      <li><span class="strategy-flow-label">How we reach them</span><h3>Invite &amp; help</h3><p>A personal invitation. A real demo.</p></li>
      <li><span class="strategy-flow-label">Then</span><h3>Repeat what works</h3><p>Improve from real feedback.</p></li>
    </ol>
    <div class="strategy-next"><span>UP NEXT</span><strong>${esc(current.label)}</strong><p>${esc(current.outcome)}</p></div>
    <details class="strategy-full-plan" ${growth.planExpanded ? 'open' : ''}><summary>See the full plan</summary><div class="strategy-full-body">
    ...
    <p class="timeline-footnote">Estimated timing. The 7- and 14-day checks start after each creator’s first export.</p></div></details>
  </section>`;
...
document.addEventListener("click", async (event) => {
  if (event.target.closest('.strategy-full-plan > summary')) growth.planExpanded = !event.target.closest('details').open;
  const timeline = event.target.closest("[data-timeline-stage], [data-open-timeline]");
  if (timeline) {
    growth.planExpanded = true;
