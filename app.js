...
function growthDiscovery() {
  ...
  const discoveryCount = value => discovery.batchCount === 0 && discovery.latest?.status === 'SUPERSEDED' ? 'Pending review' : value;
  ...
  $("#main").innerHTML = head("Creator discovery", "Batch-level discovery evidence for the existing Growth Brain. Individual prospects and contact details stay private.") + growthNavigation() + `<div class="growth-workspace discovery-workspace"><section class="discovery-intro"><div><div class="eyebrow">LATEST BATCH</div><h2>${esc(source)}</h2><p>${esc(learning)}</p></div><span class="tag neutral">${esc(latest?.status || "No batch")}</span></section><section class="discovery-stats"><article class="stat"><div class="label">Scanned</div><div class="number">${discoveryCount(discovery.scanned)}</div><div class="note">Batch observations, not unique people</div></article><article class="stat"><div class="label">Discovery leads</div><div class="number">${discoveryCount(discovery.discoveryLeads)}</div><div class="note">Batch leads; deduplicate before reporting people</div></article><article class="stat"><div class="label">Suitable</div><div class="number">${discoveryCount(discovery.suitable)}</div><div class="note">Not a current unique strong-person total</div></article><article class="stat"><div class="label">Reserve / rejected</div><div class="number">${discoveryCount(discovery.reservesRejected)}</div><div class="note">Not counted toward outreach</div></article></section><section class="panel discovery-routes"><div class="panel-head"><div><div class="eyebrow">ROUTE MIX</div><h2>Public discovery routes</h2><p>Routes are discovery signals, not permission to message.</p></div></div><div class="discovery-route-grid"><div><strong>${discoveryCount(discovery.routes.socialDm)}</strong><span>Social DM</span></div><div><strong>${discoveryCount(discovery.routes.publicEmail)}</strong><span>Public email</span></div><div><strong>${discoveryCount(discovery.routes.newRoute)}</strong><span>New route</span></div></div></section><section class="panel discovery-next"><div class="eyebrow">NEXT AI ACTION</div><p>${esc(next)}</p><small>${discovery.batchCount} completed ${discovery.batchCount === 1 ? "batch" : "batches"}. The private batch ledger and prospect records are the execution source.</small></section></div>`;
}
